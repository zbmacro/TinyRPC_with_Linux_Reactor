...
const (
	...
	ConnectTimeOut       time.Duration = time.Second * 10 // 建立连接超时时间，默认值：10s
	ClientCallTimeOut    time.Duration = time.Second * 10 // 客户端调用超时时间，默认值：10s
	ServiceHandleTimeOut time.Duration = time.Second * 10 // 服务端处理超时时间，默认值：10s
)
